...
dat=pd.read_excel('dataset_A0/data_A0_y.xlsx')
...
dat_y.iloc[0:355, :].to_excel('dataset_A0/yuce_A0.xls')


#%%
dat_s=pd.read_excel('dataset_A0/data_A0_s.xlsx')

date_s=[]

for i in range(len(dat_s['B'])):
    str_date=str(dat_s['B'][i]).split(' ')
    date_s.append(str_date[0])

dat_s['B']=date_s

dat_s=dat_s.groupby('B').mean()
dat_s=dat_s.reset_index()
dat_s.to_excel('dataset_A0/shice_A0.xls')



#%%

dat_y=pd.read_excel('dataset_A0/yuce_A0.xls')
dat_s=pd.read_excel('dataset_A0/shice_A0.xls')
...
dat_x.to_excel('dataset_A0/xiuzheng_A0.xls')
